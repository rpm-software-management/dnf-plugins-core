...
The key store can be inspected with ``/usr/lib/pqrpm/bin/rpmkeys -D
'_keyring openpgp' -D '_keyringpath %{_dbpath}/multisig' --list`` command.
A key can be deleted from the key store with ``/usr/lib/pqrpm/bin/rpmkeys -D
'_keyring openpgp' -D '_keyringpath %{_dbpath}/multisig' --delete
