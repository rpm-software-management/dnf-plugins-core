...
class ManifestCommand(dnf.cli.Command):
    ...
    def _generate(self, specs):
        ...
        if self.opts.specs:
            ...
            available_pkgs = self.base.sack.query().available()
        ...
        for dnf_pkg in dnf_pkgs:
            ...
            if dnf_pkg._is_local_pkg():
                ...
                self._setup_remote_location(pkg, dnf_pkg, available_pkgs)
            else:
                pkg.repo_id = dnf_pkg.repoid
                pkg.location = str(dnf_pkg.location)
            self._add_repository(pkg, manifest)
    ...
    def _setup_remote_location(self, pkg, dnf_pkg, available_pkgs):
        q = available_pkgs.filter(
    ...
    def _add_repository(self, pkg, manifest):
        if not pkg.repo_id in manifest.repositories:
            repository = libpkgmanifest.Repository()
            if pkg.repo_id in self.base.repos:
                dnf_repo = self.base.repos[pkg.repo_id]
                repository.id = pkg.repo_id
                repository.url = dnf_repo.remote_location('/').replace(self.base.conf.arch, '$arch')
            else:
