...
class MultiSig(dnf.Plugin):
    ...
    def __init__(self, base, cli):
        ...
        # FIXME path to the executable
        self.rpmkeys_executable = "/home/test/rpm/redhat-linux-build/tools/rpmkeys"
    ...
    def _checkSig(self, ts, package):
        ...
        try:
            value = self._verifyPackageUsingRpmkeys(fdno, ts.ts.rootDir)
        finally:
            os.close(fdno)
        return value
    ...
    def _sig_check_pkg(self, po):
        ...
        if check:
            print(_("MULTISIG checking: {}").format(po.localPkg()))
            root = self.base.conf.installroot
            # FIXME Pass installroot as a string to _checkSig() directly
            # instead of constructing
            # dnf.rpm.transaction.initReadOnlyTransaction().
            ts = dnf.rpm.transaction.initReadOnlyTransaction(root)
            sigresult = self._checkSig(ts, po.localPkg())
        ...
        print(_("MULTISIG result={}, message={}").format(result, msg))
        return result, msg
    ...
    def keyInstalled(self, fingerprint):
        ...
        print(_("MULTISIG: Checking a presence of key={}").format(fingerprint))
        args = (self.rpmkeys_executable, *self.rpmkeys_arguments,
                '--root', self.base.conf.installroot,
    ...
    def importKey(self, key):
        ...
        with subprocess.Popen(
                ...
                env=env,
                cwd='/',
                # FIXME: rpmkeys fails to read from a pipe. Fix proposed at
        ...
        if type(returncode) is not int:
            raise AssertionError('Popen set return code to non-int')
        print("MULTISIG: import result: exitcode={}, stdout={}, stderr={}".format(
    ...
    def _get_key_for_package(self, po, askcb=None, fullaskcb=None):
        ...
        for keyurl in keyurls:
            ...
            for info in keys:
                ...
                # Import the key
                # FIXME: raw_key of second info erroneously contains first and
                # second key. Probably a bug in key parser.
                #print("MULTISIG: Importing a key: {}".format(info.raw_key))
    ...
    def gpgsigcheck(self, pkgs):
        ...
        """
        error_messages = []
        # expired-pgp-keys plugin cannot know anything about our keystore.
