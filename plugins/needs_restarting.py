    ...
    def boot_time(self):
        ...
        try:
            ...
            if units_load_start_timestamp != 0:
                ...
                logger.debug(f"Got boot time from systemd: {systemd_boot_time}")
                return systemd_boot_time
        except dbus.exceptions.DBusException as e:
            logger.debug(f"D-Bus error getting boot time from systemd: {e}")
        ...
        kernel_boot_time = self.kernel_boot_time

        boot_time = max(proc_1_boot_time, kernel_boot_time)

        logger.debug(f"st_mtime of /proc/1: {proc_1_boot_time}")
        logger.debug(f"btime field of /proc/stat: {kernel_boot_time}")
        logger.debug(f"Using {boot_time} as the system boot time.")
